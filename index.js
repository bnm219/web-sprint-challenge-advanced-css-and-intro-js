...
function get20s(arr){
  const twentiethCenturyArr= [];
  for(let i =0; i < arr.length; i++){
    if(arr[i]['years'][1].includes('9')){
      twentiethCenturyArr.push(arr[i]['name']);
    }
  }
  return twentiethCenturyArr;
}
...
function addArtist(arr){
    arr.push(
      { 
        ...
        genre: 'Web Design', 
        nationality: 'USA',
        bio: 'Hi, I am not an artist but thanks haha'
      } 
    );
    return arr;
  }
...
function lotsOfArt(arr){
  ...
  for(let i =0; i < arr.length; i++){
    if(artists[i]['paintings'] > 100){
      overOneHundredArr.push(artists[i]['name'])
    }
  }
